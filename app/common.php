 ...
 * @return string sql语句
 */
if (function_exists('parseUpdate')) {
 ...
 * 转化树结构为列表结构
 */
if (!function_exists('changeTreeToListByPid')) {
    function changeTreeToListByPid($tree, $field)
    {
        if (empty($tree)) {
            return [];
        }
        ...
        foreach ($tree as $key => $value) {
            $childList = changeTreeToListByPid($value[$field], $field);
            unset($value[$field]);
            $list[] = $value;
